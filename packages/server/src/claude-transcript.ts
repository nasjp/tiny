...
/**
 * The answers of an AskUserQuestion, as Claude Code records them on the tool_result record:
 * `toolUseResult = { questions: [...], answers: { "<question text>": "<chosen label>" }, annotations? }`.
 * A rejected or malformed call records a plain string there instead, which carries no answers.
 */
function questionAnswers(toolUseResult: unknown): Record<string, string> | null {
  if (toolUseResult === null || typeof toolUseResult !== "object" || Array.isArray(toolUseResult)) return null;
  const o = toolUseResult as Record<string, unknown>;
 ...
 * `openQuestions` are AskUserQuestion tool_use ids from earlier reads that are still waiting for an
 * answer: without them a question dismissed in the CLI reads as an ordinary tool finishing, and the
 * phone's question card would never close.
 */
export function readTranscript(
  file: string,
  opts: { sinceUuid?: string | null; turns?: number; maxRecords?: number; openQuestions?: Iterable<string> } = {},
): TranscriptRead {
  const records = parseRecords(file);
  ...
  // tool_use ids of AskUserQuestion calls seen in this read. Their tool_result is the answer, not a
  // tool finishing (see the cli_question events below)
  const questionIds = new Set<string>(opts.openQuestions ?? []);
  // Questions whose tool_result this read produced an event for
  ...
  for (const r of slice) {
    ...
    if (r.type === "user") {
      ...
      for (const b of blocks(message.content)) {
        ...
        // An answered AskUserQuestion: the answers live on the record, keyed by question text.
        // Recognised by shape, so an answer whose question record fell outside this read still
        // reaches the phone as a question+answer card
        const answers = questionAnswers(r.toolUseResult);
        if (answers !== null) {
          events.push({ type: "cli_question_answered", payload: { toolUseId, answers } });
          resolvedQuestions.add(toolUseId);
          continue;
        }
        // The person dismissed the question in the CLI (or the call was malformed): no answers were
        // recorded. Still closes the question card, so it does not sit there waiting forever
        if (questionIds.has(toolUseId)) {
          events.push({ type: "cli_question_answered", payload: { toolUseId, answers: {}, rejected: true } });
          resolvedQuestions.add(toolUseId);
          continue;
        }
        events.push({
    ...
    for (const b of blocks(message.content)) {
      ...
      if (b.type === "tool_use") {
        ...
        // AskUserQuestion is the CLI asking its person something, not a tool call to narrate as
        // "Ran 1 tool". Only the CLI can answer it (the messaging socket takes messages, not
        // answers), so the phone shows it read-only and turns it into a question+answer card once
        // the tool_result carries the answers
        if (hint.kind === "question") {
          const toolUseId = typeof b.id === "string" ? b.id : "";
          questionIds.add(toolUseId);
          events.push({ type: "cli_question", payload: { toolUseId, input: b.input ?? {} } });
          continue;
        }
        events.push({
          type: "tool_started",
  ...
  // An open question from an earlier read whose result landed outside this one: before a backfill
  // window (the hook announced the question while nothing was importing the transcript — no cursor
  // yet — and the person answered long before the newest turns), or beyond a cursor the file no
  // longer has. The phone is waiting on exactly these, so they are looked up in the whole transcript.
  // They come first: they happened before anything in the window
  const pending = [...(opts.openQuestions ?? [])].filter((id) => !resolvedQuestions.has(id));
  if (pending.length > 0) {
    const settled: TranscriptEvent[] = [];
    for (const r of messages) {
      if (r.type !== "user" || r.isMeta === true) continue;
      const message = (r.message ?? {}) as Record<string, unknown>;
      for (const b of blocks(message.content)) {
        if (b.type !== "tool_result" || typeof b.tool_use_id !== "string" || !pending.includes(b.tool_use_id)) continue;
        const answers = questionAnswers(r.toolUseResult);
        settled.push(answers !== null
          ? { type: "cli_question_answered", payload: { toolUseId: b.tool_use_id, answers } }
          : { type: "cli_question_answered", payload: { toolUseId: b.tool_use_id, answers: {}, rejected: true } });
      }
    }
    events.unshift(...settled);
  }
